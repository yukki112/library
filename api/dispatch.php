...
// -----------------------------------------------------------------------------
// Robust error handling - Ensure only JSON is output
function output_json_error($message, $code = 500) {
    if (headers_sent()) {
        // If headers already sent, try to clean output
        ob_end_clean();
    }
...
set_exception_handler(function (Throwable $e): void {
    ...
    output_json_error('Server error: ' . $e->getMessage(), 500);
});
...
try {
    ...
    'fields' => ['borrow_log_id','patron_id','book_copy_id','current_due_date','requested_extension_date','extension_days','reason','status','admin_notes','extension_fee','receipt_number','approved_by','approved_at'],
    'defaults' => ['status' => 'pending'],
        ...
        'borrow_logs' => [
            'table' => 'borrow_logs',
            'fields' => ['book_id','book_copy_id','patron_id','borrowed_at','due_date','returned_at','status','notes'],
    ...
    // Handle special API endpoints BEFORE checking $RESOURCES
    $special_endpoints = ['book-details', 'book-copies', 'book-locations', 'library-sections'];
    if (in_array($resource, $special_endpoints)) {
        ...
        case 'PATCH':
            ...
            if ($resource === 'reservations') {
                ...
                if ($prevReservation && isset($data['status']) && ($row['status'] ?? '') === 'approved' && ($prevReservation['status'] ?? '') !== 'approved') {
                    ...
                    // Check if borrow log already exists for this specific copy and reservation
                    $copyId = $row['book_copy_id'] ?? null;
                    if ($copyId) {
                        $dupStmt = $pdo->prepare('SELECT id FROM borrow_logs WHERE book_id = :bid AND patron_id = :pid AND book_copy_id = :cid AND borrowed_at = :bat LIMIT 1');
                        $dupStmt->execute([
                            ':bid' => (int)$row['book_id'], 
                        ...
                        if (!$existingBorrowId) {
                            // Insert the borrow log entry with specific book_copy_id
                            $stmtInsert = $pdo->prepare('INSERT INTO borrow_logs (book_id, book_copy_id, patron_id, borrowed_at, due_date, status, notes) VALUES (:bid, :cid, :pid, :bat, :dd, :status, :notes)');
                            $stmtInsert->execute([
                                ':bid' => (int)$row['book_id'],
                                ':cid' => $copyId,
                                ':pid' => (int)$row['patron_id'],
                ...
                if (($row['status'] ?? '') === 'returned') {
                    ...
                    if (!empty($row['book_copy_id'])) {
                        $pdo->prepare('UPDATE book_copies SET status = "available" WHERE id = :cid')
                            ->execute([':cid' => (int)$row['book_copy_id']]);
                    }
